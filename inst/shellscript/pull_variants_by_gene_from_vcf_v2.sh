...
# for i in $(seq 1 22); do
  ...
  if [[ -e  ${vcf_dir}/Sardinia.b37.ss2120.FAref.impv4.chr${i}.vcf.gz  ]]; then
	  ...
	  if [[ ! -e  ${assembled_dir}/chrom_${i}/vcftools_output ]]; then
	    mkdir -p ${assembled_dir}/chrom_${i}/vcftools_output
	  fi
	  ...
	  cat ${gene_dir}/chrom_${i}.bed | \
		...
		--remove-filtered-all \
		--012 \
		--out  ${assembled_dir}/chrom_${i}/vcftools_output/{4}
    else
